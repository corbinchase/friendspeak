...
from forms import UserForm, WPostForm, PPostForm, WCommentForm, PCommentForm
from models import Wpost, Ppost, Profile, Wcomment, Pcomment, Wlike, Wdislike, Plike, Pdislike
...
def home(request):
    if mobileBrowser(request):
        if not request.user.is_authenticated():
            return render_to_response("m_register.html")
    else
        if not request.user.is_authenticated():
...
def login(request):
    username = password = profilename = profilelocale = profileage = profilepicture = profileemail = profilebio = profilelocation = ''
    if request.POST:
        ...
        if user is not None:
            ...
            newuser.save()
            profile = Profile.objects.create(user = newuser, name = profilename, locale = profilelocale, age = profileage, picture = profilepicture, bio = profilebio, location = profilelocation)
            profile.save()
            user = authenticate(username = username, password = password)
            auth_login(request, user)
            return HttpResponseRedirect('/')
